...
const selectedSource = ref<'mangaworld' | 'mangafire' | 'atsu'>('mangaworld')
...
async function startReader(id: string) {
  if (!['mangaworld:', 'mangafire:', 'atsu:'].some((prefix) => id.startsWith(prefix))) return
  readerLoading.value = true
  readerError.value = ''
  readerPages.value = []
  currentPageIndex.value = 0
  ...
  <div class="app-shell">
    ...
    <main class="main-content">
      ...
      <div class="page-wrap">
        ...
        <section v-else-if="currentView === 'search'" class="page-section"><div class="hero-row"><div><p class="eyebrow">CATALOGO · {{ sourceName.toUpperCase() }}</p><h1>Esplora manga<span class="orange-dot">.</span></h1><p class="subtitle">Risultati caricati dal backend e dalla fonte configurata.</p></div><select v-model="selectedSource" class="source-select" aria-label="Scegli fonte"><option value="mangaworld">MangaWorld · IT</option><option value="mangafire">MangaFire · EN</option><option value="atsu">Atsumaru · EN</option></select></div><div class="search-box"><span>⌕</span><input v-model="searchQuery" placeholder="Cerca per titolo, autore o genere..." /><kbd>⌘ K</kbd></div><div v-if="sourceError" class="source-alert"><span class="alert-icon">!</span><div><b>Fonte {{ sourceName }} non disponibile</b><p>{{ sourceError }}</p></div><button class="outline-button small" @click="fetchCatalog">Riprova</button></div><div v-if="catalogLoading" class="catalog-loading"><span class="spinner" /> Interrogo {{ sourceName }} tramite l'API…</div><div class="filter-row"><button v-for="tag in ['Tutti', 'Azione', 'Romance', 'Fantasy', 'Seinen', 'Sovrannaturale']" :key="tag" :class="{ selected: filter === tag }" @click="filter = tag">{{ tag }}</button><span class="results-count">{{ catalogItems.length }} risultati{{ catalogLoaded ? ' · API' : ' · demo' }}</span></div><div class="manga-grid search-grid"><article v-for="item in catalogItems" :key="item.id" class="manga-card" @click="openManga(item.id)"><div class="card-cover" :style="{ backgroundImage: `url(${item.cover})` }"><span class="status-pill">{{ item.status }}</span><button class="bookmark" @click.stop="toggleLibrary(item)">{{ item.progress > 0 ? '♥' : '＋' }}</button></div><div class="card-body"><h3>{{ item.title }}</h3><p>{{ item.author }} · {{ item.genre }}</p><div class="card-bottom"><span>{{ item.source }}</span><span>{{ item.chapters ? `${item.chapters} capitoli` : 'Catalogo API' }}</span></div></div></article></div></section>
